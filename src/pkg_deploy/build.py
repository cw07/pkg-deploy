    ...
    def create_setup_py_for_cython(config: DeployConfig, toml_config: TOMLDocument):
        ...
        if config.use_minifier:
            minify_lines = [
                ...
                "            # Minified source must not be compiled with annotation",
                "            # typing: rename_locals reuses one short name for a",
                "            # comprehension variable and an annotated local (legal",
                "            # Python -- comprehensions scope separately and local",
                "            # annotations are unenforced), but Cython inlines",
                "            # comprehensions into the function scope and enforces",
                "            # the annotation as a C type, raising e.g.",
                "            # \"TypeError: Expected list, got dict\" at runtime.",
                "            # Annotations add nothing to a minified binary wheel.",
